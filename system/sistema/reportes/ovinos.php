...
$pdf->Cell(120,12,utf8_decode('BENEFICIO DE OVINOS'),0,0,'C');
$pdf->SetFont('Arial','B',8);
...
$pdf->SetFont('Arial','B',8);
$pdf->Ln(4);
$pdf->Cell(200,5,utf8_decode('II. REGISTRO DE BENEFICIO DE OVINOS'),1,0,'L');
...
$pdf->Cell(30,5,utf8_decode('OVEJA'),0,0,'C');
$pdf->Cell(30,5,utf8_decode('CORDERO'),0,0,'C');
$pdf->Cell(30,5,utf8_decode('OVINO ADULTO'),0,0,'C');
$pdf->Cell(50,5,utf8_decode(''),0,0,'C');
$pdf->Cell(30,5,utf8_decode('OVEJA'),0,0,'C');
$pdf->Cell(30,5,utf8_decode('CORDERO'),0,0,'C');
$pdf->Cell(30,5,utf8_decode('OVINO ADULTO'),0,1,'C');
...
$cuenta=0;
	while ($resultd = mysqli_fetch_array($queryd)){	
		...
		$queryd1 = mysqli_query("Select (SELECT SUM(pd.prdes_unidad) as prdes_unidad FROM produccion_destino pd, rubro_tipo rt where pd.prdes_prcodigo=".$result['pr_codigo']." and pd.prdes_rutcodigo=rt.rut_codigo and pd.prdes_entidad='".$resultd['prdes_entidad']."' AND rt.rut_descripcion='OVEJA') as oveja,(SELECT SUM(pd.prdes_unidad) as prdes_unidad FROM produccion_destino pd, rubro_tipo rt where pd.prdes_prcodigo=".$result['pr_codigo']." and pd.prdes_rutcodigo=rt.rut_codigo and pd.prdes_entidad='".$resultd['prdes_entidad']."' AND rt.rut_descripcion='CORDERO') as cordero ,(SELECT SUM(pd.prdes_unidad) as prdes_unidad FROM produccion_destino pd, rubro_tipo rt where pd.prdes_prcodigo=".$result['pr_codigo']." and pd.prdes_rutcodigo=rt.rut_codigo and pd.prdes_entidad='".$resultd['prdes_entidad']."' AND rt.rut_descripcion='OVINO ADULTO') as ovinoa");
		$resultd1 = mysqli_fetch_array($queryd1);
		$pdf->Cell(30,5,utf8_decode($resultd1['oveja']/1),1,0,'C');
		$pdf->Cell(30,5,utf8_decode($resultd1['cordero']/1),1,0,'C');
		$pdf->Cell(30,5,utf8_decode($resultd1['ovinoa']/1),1,1,'C');
}
...
$pdf->Ln(-$cuenta);
	while ($resultd = mysqli_fetch_array($queryd)){	
		...
		$queryd1 = mysqli_query("Select (SELECT SUM(pd.prpr_unidad) as prpr_unidad FROM produccion_procede pd, rubro_tipo rt where pd.prpr_prcodigo=".$result['pr_codigo']." and pd.prpr_rutcodigo=rt.rut_codigo and pd.prpr_entidad='".$resultd['prpr_entidad']."' AND rt.rut_descripcion='OVEJA') as oveja,(SELECT SUM(pd.prpr_unidad) as prpr_unidad FROM produccion_procede pd, rubro_tipo rt where pd.prpr_prcodigo=".$result['pr_codigo']." and pd.prpr_rutcodigo=rt.rut_codigo and pd.prpr_entidad='".$resultd['prpr_entidad']."' AND rt.rut_descripcion='CORDERO') as cordero ,(SELECT SUM(pd.prpr_unidad) as prpr_unidad FROM produccion_procede pd, rubro_tipo rt where pd.prpr_prcodigo=".$result['pr_codigo']." and pd.prpr_rutcodigo=rt.rut_codigo and pd.prpr_entidad='".$resultd['prpr_entidad']."' AND rt.rut_descripcion='OVINO ADULTO') as ovinoa");
		$resultd1 = mysqli_fetch_array($queryd1);
		$pdf->Cell(30,5,utf8_decode($resultd1['oveja']/1),1,0,'C');
		$pdf->Cell(30,5,utf8_decode($resultd1['cordero']/1),1,0,'C');
		$pdf->Cell(30,5,utf8_decode($resultd1['ovinoa']/1),1,1,'C');
