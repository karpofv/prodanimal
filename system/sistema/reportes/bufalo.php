...
$pdf->Cell(120,12,utf8_decode('BENEFICIO DE BUFALOS SIN CATEGORÍA'),0,0,'C');
$pdf->SetFont('Arial','B',8);
...
$pdf->SetFont('Arial','B',8);
$pdf->Cell(280,5,utf8_decode('DIRECCIÓN'),1,1,'L');
$pdf->SetFont('Arial','',6);
$pdf->Cell(280,5,utf8_decode($result['est_direccion']),1,1,'L');
$pdf->Ln(4);
$pdf->Cell(200,5,utf8_decode('II. REGISTRO DE BENEFICIO DE BUFALOS'),1,1,'L');
$pdf->SetFont('Arial','',8);
...
$pdf->Cell(50,5,utf8_decode('PESO PROMEDIO'),1,0,'C');
...
$cuenta=0;
	while ($resultd = mysqli_fetch_array($queryd)){	
		...
		$queryd1 = mysqli_query("Select SUM(pd.prdes_unidad) as totalk FROM produccion_destino pd, rubro_tipo rt where pd.prdes_prcodigo=".$result['pr_codigo']." and pd.prdes_rutcodigo=rt.rut_codigo and pd.prdes_entidad='".$resultd['prdes_entidad']."' and pd.prdes_estado='ACTIVO'");
		$resultd1 = mysqli_fetch_array($queryd1);
		$pdf->Cell(62,5,utf8_decode($resultd1['totalk']/1),1,1,'C');		
}
...
$pdf->Ln(-$cuenta);
	while ($resultd2 = mysqli_fetch_array($queryd)){	
		$pdf->Cell(50,5,utf8_decode($resultd2['prpr_entidad']),1,0,'C');
		$queryd1 = mysqli_query("Select (SELECT SUM(pd.prpr_unidad) as prpr_unidad FROM produccion_procede pd, rubro_tipo rt where pd.prpr_prcodigo=".$result['pr_codigo']." and pd.prpr_rutcodigo=rt.rut_codigo and pd.prpr_entidad='".$resultd2['prpr_entidad']."' AND rt.rut_descripcion='MACHO' and pd.prpr_estado='ACTIVO') as cabezam,(SELECT SUM(pd.prpr_unidad) as prpr_unidad FROM produccion_procede pd, rubro_tipo rt where pd.prpr_prcodigo=".$result['pr_codigo']." and pd.prpr_rutcodigo=rt.rut_codigo and pd.prpr_entidad='".$resultd2['prpr_entidad']."' and rt.rut_descripcion='HEMBRA' and pd.prpr_estado='ACTIVO') as cabezah");
		$resultd1 = mysqli_fetch_array($queryd1);
		$pdf->Cell(40,5,utf8_decode($resultd1['cabezam']/1),1,0,'C');
		$pdf->Cell(40,5,utf8_decode($resultd1['cabezah']/1),1,0,'C');
		$pdf->Cell(38,5,utf8_decode(($resultd1['cabezam']+$resultd1['cabezah'])/1),1,1,'C');		
	}
$pdf->Ln(15);
$pdf->SetFont('Arial','B',8);
...
$pdf->Cell(80,12,utf8_decode(''),0,0,'C');
$pdf->SetFont('Arial','B',12);
$pdf->Cell(120,12,utf8_decode('BENEFICIO DE BUFALOS POR CATEGORÍA'),0,0,'C');
$pdf->SetFont('Arial','B',8);
$pdf->Cell(80,5,utf8_decode('I. IDENTIFICACIÒN'),1,1,'L');
$pdf->Cell(80,12,utf8_decode(''),0,0,'C');
$pdf->Cell(120,5,utf8_decode(''),0,0,'C');
...
$pdf->SetFont('Arial','B',8);
$pdf->Ln(4);
$pdf->Cell(280,5,utf8_decode('II. BENEFICIO DE BUFALOS POR CATEGORIA'),1,1,'L');
$pdf->SetFont('Arial','',8);
...
$pdf->Cell(19,5,utf8_decode('PESO(KG)'),1,0,'C');
$pdf->Ln(5);
$pdf->Cell(15,15,utf8_decode('MACHO'),1,1,'C');
$pdf->Cell(15,15,utf8_decode('HEMBRA'),1,1,'C');
$pdf->Ln(-30);
$query=mysqli_query("SELECT rc.ruc_descripcion from rubro_calse rc, rubros r WHERE r.ru_codigo=rc.ruc_rucodigo and r.ru_descripcion='BUFALO'");
while($resultclas=mysqli_fetch_array($query)){
	...
	$querycat=mysqli_query("SELECT rc.rucat_descripcion from rubro_categoria rc, rubros r WHERE r.ru_codigo=rc.rucat_rucodigo and r.ru_descripcion='BUFALO'");
