...
$pdf->Cell(120,12,utf8_decode('BENEFICIO DE CAPRINOS'),0,0,'C');
$pdf->SetFont('Arial','B',8);
...
$pdf->SetFont('Arial','B',8);
$pdf->Ln(4);
$pdf->Cell(200,5,utf8_decode('II. REGISTRO DE BENEFICIO DE CAPRINOS'),1,0,'L');
...
while ($resultp = mysqli_fetch_array($queryp)){
	...
	$pdf->Cell(50,5,utf8_decode($resultd['prd_cantidad']),1,0,'C');
	...
	$resultd = mysqli_fetch_array($queryd);	
	$pdf->Cell(50,5,utf8_decode($resultd['prd_cantidad']),1,1,'C');
}
$pdf->Ln(5);
$pdf->SetFont('Arial','B',8);
...
$pdf->Cell(30,5,utf8_decode('CABRA'),0,0,'C');
$pdf->Cell(30,5,utf8_decode('CABRITO'),0,0,'C');
$pdf->Cell(30,5,utf8_decode('CAPRINO ADULTO'),0,0,'C');
$pdf->Cell(50,5,utf8_decode(''),0,0,'C');
$pdf->Cell(30,5,utf8_decode('CABRA'),0,0,'C');
$pdf->Cell(30,5,utf8_decode('CABRITO'),0,0,'C');
$pdf->Cell(30,5,utf8_decode('CAPRINO ADULTO'),0,1,'C');
...
$cuenta=0;
	while ($resultd = mysqli_fetch_array($queryd)){	
		...
		$queryd1 = mysqli_query("Select (SELECT SUM(pd.prdes_unidad) as prdes_unidad FROM produccion_destino pd, rubro_tipo rt where pd.prdes_prcodigo=".$result['pr_codigo']." and pd.prdes_rutcodigo=rt.rut_codigo and pd.prdes_entidad='".$resultd['prdes_entidad']."' AND rt.rut_descripcion='CABRA') as cabra,(SELECT SUM(pd.prdes_unidad) as prdes_unidad FROM produccion_destino pd, rubro_tipo rt where pd.prdes_prcodigo=".$result['pr_codigo']." and pd.prdes_rutcodigo=rt.rut_codigo and pd.prdes_entidad='".$resultd['prdes_entidad']."' AND rt.rut_descripcion='CABRITO') as cabrito ,(SELECT SUM(pd.prdes_unidad) as prdes_unidad FROM produccion_destino pd, rubro_tipo rt where pd.prdes_prcodigo=".$result['pr_codigo']." and pd.prdes_rutcodigo=rt.rut_codigo and pd.prdes_entidad='".$resultd['prdes_entidad']."' AND rt.rut_descripcion='CAPRINO ADULTO') as caprinoa");
		$resultd1 = mysqli_fetch_array($queryd1);
		$pdf->Cell(30,5,utf8_decode($resultd1['cabra']/1),1,0,'C');
		$pdf->Cell(30,5,utf8_decode($resultd1['cabrito']/1),1,0,'C');
		$pdf->Cell(30,5,utf8_decode($resultd1['caprinoa']/1),1,1,'C');
}
...
$pdf->Ln(-$cuenta);
	while ($resultd = mysqli_fetch_array($queryd)){	
		...
		$queryd1 = mysqli_query("Select (SELECT SUM(pd.prpr_unidad) as prpr_unidad FROM produccion_procede pd, rubro_tipo rt where pd.prpr_prcodigo=".$result['pr_codigo']." and pd.prpr_rutcodigo=rt.rut_codigo and pd.prpr_entidad='".$resultd['prpr_entidad']."' AND rt.rut_descripcion='CABRA') as cabra,(SELECT SUM(pd.prpr_unidad) as prpr_unidad FROM produccion_procede pd, rubro_tipo rt where pd.prpr_prcodigo=".$result['pr_codigo']." and pd.prpr_rutcodigo=rt.rut_codigo and pd.prpr_entidad='".$resultd['prpr_entidad']."' AND rt.rut_descripcion='CABRITO') as cabrito ,(SELECT SUM(pd.prpr_unidad) as prpr_unidad FROM produccion_procede pd, rubro_tipo rt where pd.prpr_prcodigo=".$result['pr_codigo']." and pd.prpr_rutcodigo=rt.rut_codigo and pd.prpr_entidad='".$resultd['prpr_entidad']."' AND rt.rut_descripcion='CAPRINO ADULTO') as caprinoa");
		$resultd1 = mysqli_fetch_array($queryd1);
		$pdf->Cell(30,5,utf8_decode($resultd1['cabra']/1),1,0,'C');
		$pdf->Cell(30,5,utf8_decode($resultd1['cabrito']/1),1,0,'C');
		$pdf->Cell(30,5,utf8_decode($resultd1['caprinoa']/1),1,1,'C');
