...
$pdf->Cell(120,12,utf8_decode('BENEFICIO DE BOVINOS SIN CATEGORÍA'),0,0,'C');
$pdf->SetFont('Arial','B',8);
...
$pdf->SetFont('Arial','B',8);
$pdf->Ln(4);
$pdf->Cell(200,5,utf8_decode('II. REGISTRO DE BENEFICIO DE BOVINOS'),1,0,'L');
...
$pdf->Cell(25,5,utf8_decode('11.- EN CANAL'),1,0,'C');
$pdf->Ln(5);
$queryp = mysqli_query("SELECT rt.rut_descripcion,SUM(pd.prd_cantidad)  as cantidad  from produccion_detalle pd, produccion p, rubro_tipo rt where p.pr_codigo=pd.prd_prcodigo and p.pr_codigo=".$result['pr_codigo']." and pd.prd_rutcodigo=rt.rut_codigo and pd.prd_estado='ACTIVA' GROUP BY rt.rut_descripcion");
...
$cuenta=0;
	while ($resultd = mysqli_fetch_array($queryd)){	
		$cuenta=$cuenta+5;
		$pdf->Cell(168,5,utf8_decode(''),1,0,'C');		
		$pdf->Cell(50,5,utf8_decode($resultd['prdes_entidad']),1,0,'C');
		$queryd1 = mysqli_query("Select (SELECT SUM(pd.prdes_unidad) as prdes_unidad FROM produccion_destino pd, rubro_tipo rt where pd.prdes_prcodigo=".$result['pr_codigo']." and pd.prdes_rutcodigo=rt.rut_codigo and pd.prdes_entidad='".$resultd['prdes_entidad']."' AND rt.rut_descripcion='MACHO')+(SELECT SUM(pd.prdes_unidad) as prdes_unidad FROM produccion_destino pd, rubro_tipo rt where pd.prdes_prcodigo=".$result['pr_codigo']." and pd.prdes_rutcodigo=rt.rut_codigo and pd.prdes_entidad='".$resultd['prdes_entidad']."' AND rt.rut_descripcion='HEMBRA') as totalk");
		$resultd1 = mysqli_fetch_array($queryd1);
		$pdf->Cell(62,5,utf8_decode($resultd1['totalk']),1,1,'C');		
}
...
$pdf->Ln(-$cuenta);
	while ($resultd1 = mysqli_fetch_array($queryd)){	
		$pdf->Cell(50,5,utf8_decode($resultd1['prpr_entidad']),1,0,'C');
		$queryd1 = mysqli_query("Select (SELECT SUM(pd.prpr_unidad) as prpr_unidad FROM produccion_procede pd, rubro_tipo rt where pd.prpr_prcodigo=".$result['pr_codigo']." and pd.prpr_rutcodigo=rt.rut_codigo and pd.prpr_entidad='".$resultd1['prpr_entidad']."' AND rt.rut_descripcion='MACHO') as cabezam,(SELECT SUM(pd.prpr_unidad) as prpr_unidad FROM produccion_procede pd, rubro_tipo rt where pd.prpr_prcodigo=".$result['pr_codigo']." and pd.prpr_rutcodigo=rt.rut_codigo and pd.prpr_entidad='".$resultd1['prpr_entidad']."' AND rt.rut_descripcion='HEMBRA') as cabezah");
		$resultd1 = mysqli_fetch_array($queryd1);
		$pdf->Cell(40,5,utf8_decode($resultd1['cabezam']/1),1,0,'C');
		$pdf->Cell(40,5,utf8_decode($resultd1['cabezah']/1),1,0,'C');
		$pdf->Cell(38,5,utf8_decode($resultd1['cabezam']/1+$resultd1['cabezam']),1,1,'C');		
	}
$pdf->Ln(15);
$pdf->SetFont('Arial','B',8);
...
$pdf->Cell(80,12,utf8_decode(''),0,0,'C');
$pdf->SetFont('Arial','B',12);
$pdf->Cell(120,12,utf8_decode('BENEFICIO DE BOVINOS POR CATEGORÍA'),0,0,'C');
$pdf->SetFont('Arial','B',8);
$pdf->Cell(80,5,utf8_decode('I. IDENTIFICACIÒN'),1,1,'L');
$pdf->Cell(80,12,utf8_decode(''),0,0,'C');
$pdf->Cell(120,5,utf8_decode(''),0,0,'C');
...
$pdf->SetFont('Arial','B',8);
$pdf->Ln(4);
$pdf->Cell(280,5,utf8_decode('II. BENEFICIO DE BOVINOS POR CATEGORIA'),1,1,'L');
$pdf->SetFont('Arial','',8);
$pdf->Cell(50,15,utf8_decode('7.- CLASE'),1,0,'C');
$pdf->Cell(228,5,utf8_decode('CATEGORIAS'),1,1,'C');
$pdf->Cell(50,5,utf8_decode(''),0,0,'C');
$pdf->Cell(38,5,utf8_decode('TERNERA'),1,0,'C');
$pdf->Cell(38,5,utf8_decode('AA'),1,0,'C');
$pdf->Cell(38,5,utf8_decode('A'),1,0,'C');
$pdf->Cell(38,5,utf8_decode('B'),1,0,'C');
$pdf->Cell(38,5,utf8_decode('C'),1,0,'C');
$pdf->Cell(38,5,utf8_decode('D'),1,1,'C');
$pdf->Cell(50,10,utf8_decode(''),0,0,'C');
$pdf->Cell(19,5,utf8_decode('CABEZAS'),1,0,'C');
$pdf->Cell(19,5,utf8_decode('PESO(KG)'),1,0,'C');
$pdf->Cell(19,5,utf8_decode('CABEZAS'),1,0,'C');
$pdf->Cell(19,5,utf8_decode('PESO(KG)'),1,0,'C');
$pdf->Cell(19,5,utf8_decode('CABEZAS'),1,0,'C');
$pdf->Cell(19,5,utf8_decode('PESO(KG)'),1,0,'C');
...
$pdf->Cell(19,5,utf8_decode('PESO(KG)'),1,0,'C');
$pdf->Ln(5);
$pdf->Cell(15,30,utf8_decode('MACHO'),1,1,'C');
$pdf->Cell(15,20,utf8_decode('HEMBRA'),1,1,'C');
$pdf->Ln(-50);
$query=mysqli_query("SELECT rc.ruc_descripcion from rubro_calse rc, rubros r WHERE r.ru_codigo=rc.ruc_rucodigo and r.ru_descripcion='BOVINO'");
while($resultclas=mysqli_fetch_array($query)){
	$pdf->Cell(15,30,utf8_decode(''),0,0,'C');
	$pdf->Cell(35,5,utf8_decode($resultclas['ruc_descripcion']),1,0,'C');
	$querycat=mysqli_query("SELECT rc.rucat_descripcion from rubro_categoria rc, rubros r WHERE r.ru_codigo=rc.rucat_rucodigo and r.ru_descripcion='BOVINO'");
	while($resultcat=mysqli_fetch_array($querycat)){
		$queryr=mysqli_query("SELECT PD.prd_cantidad,SUM(pm.prmov_cantidad) as prmov_cantidad
from produccion_movimiento pm, produccion_detalle pd, rubro_tipo rt, tipos_medidas t, rubro_calse rc, rubro_categoria rcc
WHERE pm.prmov_prdcodigo=pd.prd_codigo and pd.prd_prcodigo=".$result['pr_codigo']." and pd.prd_rutcodigo=rt.rut_codigo and pm.prmov_tipcodigo=t.tip_codigo and pd.prd_ruccodigo=rc.ruc_codigo and pd.prd_rucatcodigo=rcc.rucat_codigo and rc.ruc_descripcion='".$resultclas['ruc_descripcion']."' and rcc.rucat_descripcion='".$resultcat['rucat_descripcion']."' and pm.prmov_estado='ACTIVA'");		
		$resultr=mysqli_fetch_array($queryr);
		$pdf->Cell(19,5,utf8_decode($resultr['prd_cantidad']/1),1,0,'C');
		$pdf->Cell(19,5,utf8_decode($resultr['prmov_cantidad']/1),1,0,'C');		
	}
	$pdf->Cell(20,5,utf8_decode(''),0,1,'C');		
}
$pdf->Ln(15);
$pdf->SetFont('Arial','B',8);
$pdf->Cell(260,5,utf8_decode('III. PROCEDENCIA Y DESTINO DE LA PRODUCCION'),1,1,'L');
$pdf->Cell(130,5,utf8_decode('12.- PROCEDENCIA DE LA PRODUCCION'),1,0,'C');
$pdf->Cell(130,5,utf8_decode('13.- DESTINO DE LA PRODUCCION'),1,1,'C');
$pdf->SetFont('Arial','B',6);
$pdf->Cell(50,5,utf8_decode('ENTIDAD / PAIS'),1,0,'C');
$pdf->Cell(40,5,utf8_decode('CABEZAS DE MACHOS'),1,0,'C');
$pdf->Cell(40,5,utf8_decode('CABEZAS DE HEMBRAS'),1,0,'C');
$pdf->Cell(50,5,utf8_decode('ENTIDAD / PAIS'),1,0,'C');
$pdf->Cell(40,5,utf8_decode('CABEZAS DE MACHOS'),1,0,'C');
$pdf->Cell(40,5,utf8_decode('CABEZAS DE HEMBRAS'),1,1,'C');
$queryd = mysqli_query("SELECT pdes.prdes_entidad from produccion_destino pdes WHERE pdes.prdes_prcodigo=".$result['pr_codigo']." and pdes.prdes_estado='ACTIVO' group by pdes.prdes_entidad");
$cuenta=0;
	while ($resultd = mysqli_fetch_array($queryd)){	
		$cuenta=$cuenta+5;
		...
		$pdf->Cell(50,5,utf8_decode($resultd['prdes_entidad']),1,0,'C');
		$queryd1 = mysqli_query("Select (SELECT SUM(pd.prdes_unidad) as prdes_unidad FROM produccion_destino pd, rubro_tipo rt where pd.prdes_prcodigo=".$result['pr_codigo']." and pd.prdes_rutcodigo=rt.rut_codigo and pd.prdes_entidad='".$resultd['prdes_entidad']."' AND rt.rut_descripcion='MACHO') as cabezam,(SELECT SUM(pd.prdes_unidad) as prdes_unidad FROM produccion_destino pd, rubro_tipo rt where pd.prdes_prcodigo=".$result['pr_codigo']." and pd.prdes_rutcodigo=rt.rut_codigo and pd.prdes_entidad='".$resultd['prdes_entidad']."' AND rt.rut_descripcion='HEMBRA') as cabezah");
		$resultd1 = mysqli_fetch_array($queryd1);
		$pdf->Cell(40,5,utf8_decode($resultd1['cabezam']/1),1,0,'C');
		$pdf->Cell(40,5,utf8_decode($resultd1['cabezah']/1),1,1,'C');
}
$queryd = mysqli_query("SELECT pdes.prpr_entidad from produccion_procede pdes WHERE pdes.prpr_prcodigo=".$result['pr_codigo']." and pdes.prpr_estado='ACTIVO' group by pdes.prpr_entidad");
$pdf->Ln(-$cuenta);
	while ($resultd1 = mysqli_fetch_array($queryd)){	
		$pdf->Cell(50,5,utf8_decode($resultd1['prpr_entidad']),1,0,'C');
		$queryd1 = mysqli_query("Select (SELECT SUM(pd.prpr_unidad) as prpr_unidad FROM produccion_procede pd, rubro_tipo rt where pd.prpr_prcodigo=".$result['pr_codigo']." and pd.prpr_rutcodigo=rt.rut_codigo and pd.prpr_entidad='".$resultd1['prpr_entidad']."' AND rt.rut_descripcion='MACHO') as cabezam,(SELECT SUM(pd.prpr_unidad) as prpr_unidad FROM produccion_procede pd, rubro_tipo rt where pd.prpr_prcodigo=".$result['pr_codigo']." and pd.prpr_rutcodigo=rt.rut_codigo and pd.prpr_entidad='".$resultd1['prpr_entidad']."' AND rt.rut_descripcion='HEMBRA') as cabezah");
		$resultd2 = mysqli_fetch_array($queryd1);
		$pdf->Cell(40,5,utf8_decode($resultd2['cabezam']/1),1,0,'C');
		$pdf->Cell(40,5,utf8_decode($resultd2['cabezah']/1),1,1,'C');
	}
$pdf->Ln(15);
$pdf->SetFont('Arial','B',8);
$pdf->Cell(280,5,utf8_decode('IV.- DATOS DEL INFORMANTE'),1,1,'L');
